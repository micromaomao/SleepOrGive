...
export class RateLimit {
	 ...
	 */
	public async bump(db?: DBClient, req_evt?: RequestEvent): Promise<RateLimitBumpResponse> {
		if (!db) {
		...
		if (req_evt) {
			let existing_rem = parseInt(req_evt.request.headers.get('RateLimit-Remaining'));
			if (Number.isNaN(existing_rem) || existing_rem > remaining) {
				req_evt.setHeaders({
					'RateLimit-Limit': this.limit.toString(),
					'RateLimit-Remaining': remaining.toString(),
					'RateLimit-Reset': reset_remaining_secs.toString()
				});
			}
		}
		return ret;
