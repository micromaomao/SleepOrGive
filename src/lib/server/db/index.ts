...
import INIT_SQL from './client_init.sql?raw';
// const DROP_EVERYTHING_SQL = read_to_string('drop_everything.sql');
import DROP_EVERYTHING_SQL from './drop_everything.sql?raw';
...
export async function init() {
	...
	try {
		...
		if (db_version != APPLICATION_DB_VERSION) {
			throw new Error(
				`Database version mismatch: expected ${APPLICATION_DB_VERSION}, got ${db_version}. Database migrations required.`
			);
		}
	} finally {
		client.release();
